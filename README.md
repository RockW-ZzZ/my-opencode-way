...
本仓库为 git 仓库（`main` 分支），远端：<https://github.com/RockW-ZzZ/my-opencode-way.git>。
...
2. **放置全局配置**
   - 把 `configs/` 下的 `opencode.json`、`cli.json`、`dcp.jsonc` 复制到本机全局目录
     ...
     - opencode **V2**（当前）：server 插件声明在 `opencode.json`，CLI 插件（如 visual-cache）声明在 `cli.json`。
     - opencode **V1**：改用 `tui.jsonc` 声明 TUI 插件（V2 首次启动会自动迁移为 `cli.json`）。
...
4. **安装外部系统依赖**（如仍需使用已落盘的嵌入式 Skill），见 `tools.md`。
5. **启动 opencode**，插件即被自动加载：
   - `@tarquinen/opencode-dcp`（动态上下文裁剪）与 `opencode-visual-cache`（CLI 缓存面板）；
   - `image-gen` 为手动 Skill，随 `skills/` 目录生效；
   - `oh-my-embedded` 在 V2 不再加载（V1 插件 API），其生成的 Skill/Agent/Command 文件仍留在磁盘，
     但 `embedded-*` 工具不可用，见 `plugins.md`。
